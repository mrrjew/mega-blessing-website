...
async function listTable(tableName: string) {
    ...
    try {
        ...
        if (records.length === 0) {
            ...
            console.log(JSON.stringify(records.map((r: any) => ({ id: r.id, ...r.fields })), null, 2));
        }
    } catch (error: any) {
...
if (command === "--table" && param) {
    ...
    console.log("Usage:");
    console.log("  npx ts-node scripts/inspect.ts --table <TableName>");
    console.log("  npx ts-node scripts/inspect.ts --webhooks");
}
